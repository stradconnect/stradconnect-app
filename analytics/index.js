(function () {
  ...
  function renderStorage(data) {
    S.loadStorage(true, S.buildValidPaths(data.drawings)).then(function (st) {
      ...
      if (!kpisEl) return;
      var dbFileCount = data.drawings ? data.drawings.length : 0;
      var storageFileCount = st.files || 0;
      var defs = [
        { label: "Storage Used", value: S.fmtBytes(st.totalBytes), sub: "DB-tracked files only", gold: true },
        { label: "Files in Storage", value: storageFileCount, sub: "matches " + dbFileCount + " DB files", gold: false }
      ];
